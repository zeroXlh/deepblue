...
function operateFormat(value, row, index) {
    var s = '';
    // 修改
    //s += '<button type="button" class="btn btn-info btn-sm" onClick="openViewDialog('+ row.id + ')"></button>';
    s += "<button type=\"button\" class=\"btn btn-info btn-sm\" data-toggle=\"modal\" "
        + "data-target=\"#customerModal\" onClick=\"openViewDialog(" + row.id + ");\">客户列表</button>&nbsp;&nbsp;";
...
function saveEmployee() {
    ...
    json.idNoVerify = true;
    //json.idNoVerify = $("#idNoVerify:checked").length>0;
