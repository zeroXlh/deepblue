...
$(function () {
    ...
    $("#payDateBegin").val(new Date().format("yyyy-MM-dd"));
    var defaultEndDate = new Date();
    defaultEndDate.setDate(new Date().getDate() + 7);
    $("#payDateEnd").val(defaultEndDate.format("yyyy-MM-dd"));
...
var columns = [
    ...
    {"field": "bankInfo", "title": "银行信息", "align": "center"},
    {"field": "bankNo", "title": "银行行号", "align": "center"},
...
function loadCalendar() {
    $('#tabCalendarPlan').bootstrapTable('destroy');
    $('#tabCalendarPlan').bootstrapTable({
        url: "/hoper/backweb/plan/planCalendarByDate",
        dataType: "json",
        method: "GET",
        striped: true,//是否显示行间隔色
        cache: false,
        ...
        queryParams: function (params) {
            var jsonData = {
                ...
                payDate: $("#planDate").val(),
                status: 1
            };
            return jsonData;
        },//设置查询时候的参数，传递参数（*）
        columns: [{"field": "productName", "title": "项目名", "align": "center"},
            {"field": "period", "title": "期数", "align": "center"},
            {"field": "payQuarter", "title": "回款季度", "align": "center"},
            {"field": "payAmountAmt", "title": "回款金额", "align": "center", "formatter": floatFormat},
            {"field": "payCount", "title": "回款笔数", "align": "center"},
            {"field": "payDate", "title": "回款日期", "align": "center", "formatter": jsonDateFormat},
            {"field": "void", "title": "操作", "align": "left", "formatter": function(value, row, index){
                return '<button type="button" class="btn btn-danger btn-sm" onClick="paymentQuarter('
                    + row.productId + ',' + row.payQuarter + ')">确认回款</button>';
            }}
        ],
        showFooter: true,
        responseHandler: handler//在ajax请求成功后，发放数据之前可以对返回的数据进行处理，渲染表格前的方法
    ...
    //表格变色
    $(function () {
        $("#tabCalendartPlan tr").click(function () {
            $("#tabCalendartPlan tr").css("background", "#fff");
            $(this).css("background", "#B4E4E9");
        })
    });
}

function paymentQuarter(productId,payQuarter) {
    if (confirm("确认已给客户回款吗？")) {
        jQuery.post("/hoper/backweb/plan/paymentQuarter", {
            "productId": productId,
            "payQuarter": payQuarter
        }, function (data) {
            if (1 == data.code) {
                $("#tabCalendarPlan").bootstrapTable("refresh");
                alert("还款成功");
            } else if (0 == data.code) {
                alert(data.msg);
            } else {
...
function paymentByDate(){
    if (!$("#planDate").val()) {
        $("#planDate").val(new Date().format("yyyy-MM-dd"))
    }

    if (confirm("确认" +$("#planDate").val()+ "的客户回款已回吗？")) {
        jQuery.post("/hoper/backweb/plan/paymentByDate", {
            "payDate": $("#planDate").val()
        }, function (data) {
            if (1 == data.code) {
                $("#tabCalendarPlan").bootstrapTable("refresh");
